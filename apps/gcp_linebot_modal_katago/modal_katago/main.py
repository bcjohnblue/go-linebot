...
# HTTP/HTTPS endpoint for review
# This allows calling the function via HTTP POST request
@app.function(
    image=image,
    gpu="T4",  # KataGo needs GPU
    timeout=3600,  # 1 hour timeout
...
async def review_http(request: Request) -> Dict[str, Any]:
    """
    HTTP/HTTPS endpoint for KataGo review analysis.

    This endpoint accepts POST requests with JSON body:
    {
        "task_id": "unique-task-id",
        "sgf_gcs_path": "gs://bucket/path/to/file.sgf",
        "callback_url": "https://cloud-run-url/callback/review",
        "target_id": "line-user-id",
        "visits": 5  // optional, default: 5
    }
    ...
    try:
        ...
        if auth != f"Bearer {os.environ['API_KEY']}":
            raise HTTPException(status_code=401, detail="Unauthorized")

        # Parse JSON request body (async in FastAPI)
        ...
        if not all([task_id, sgf_gcs_path, callback_url, target_id]):
            return {
                "status": "error",
                "error": "Missing required parameters: task_id, sgf_gcs_path, callback_url, target_id",
            }

        # Call the original review function asynchronously
        # Note: We need to spawn it as a background task
        review.spawn(
            task_id=task_id,
        ...
        return {
            "status": "accepted",
            "task_id": task_id,
            "message": "Review started. Results will be sent to callback_url when complete.",
        }
