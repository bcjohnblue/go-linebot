...
def run_analysis_script(sgf_path: str, visits: int = None, *additional_args):
    """Run analysis script"""
    # Resolve SGF file path
    ...
    # Build script path
    script_path = katago_dir / "scripts" / "analysis.sh"
    ...
    if return_code == 0:
        ...
        raise RuntimeError(f"腳本異常結束，代碼: {return_code}")
...
def main():
    ...
    if len(args) == 0:
        ...
        print("使用方式: python3 analysis.py <sgf_file_path> [additional_args...]", file=sys.stderr)
        print("範例: python3 analysis.py ./static/example.sgf", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        run_analysis_script(sgf_path, visits, *additional_args)
        print("現在可以處理接下來的事情了。")
